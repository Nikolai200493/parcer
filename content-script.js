    ...
    async function startCollectData() {
      ...
      if (latestDate > oldDataDates[0] || oldDataDates[0] === undefined) {
        function openAndParsePage(link) {
          ...
          // Флаг для проверки на загрузку страницы и последующее ее закрытие
              ...
              if (procedureTitle) {
                ...
                purchaseName.forEach((item) => {
                  if (
                    ...
                    ] = `tektorg-${item.children[1].textContent.trim()}`
                  } else {
                ...
                const procedureObject = {
                  ...
                  ...organizerObject,
                  ...otherObject,
                  ['Ссылка']: newPage.location.href,
                  ['Наименование продукции']: productDescription
                }

                const isDublicate = oldData.find(
                  (item) =>
                    item['Номер процедуры'] ===
                    procedureObject['Номер процедуры']
                )
                if (!isDublicate) {
